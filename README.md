...
`Vendor/semantic-prompt-contract` version `3.1.1` package. This path is a checkout of a separate
...
The Universal AI Connector profile supports older clients that reject additive reasoning metadata. It removes `reasoning`, `reasoning_content`,
and `reasoning_details` from assistant messages and streaming deltas without merging them into
visible content. Every key requires the response model to equal the exact requested model.
The only model-identity normalization applies when a requested Ollama alias has one terminal
...
| `invalid_upstream_response` | A successful upstream response did not satisfy the guaranteed non-streaming contract, including any disallowed response-model mismatch for any key. |
| `invalid_stream` | The upstream stream had an unsupported content type, event, chunk, order, or termination, or a disallowed response-model mismatch for any key. |
