...
The preceding pass-through statement applies to Ollama and Apfel. For the privileged Codex alias, the supported request fields are `model`, `messages`, optional `stream: false`, optional `response_format: {"type":"json_object"}`, and optional OpenKeyboard `operation` plus `input_text`. Messages must use `system`, `developer`, `user`, or `assistant` roles with string content. JSON-object mode validates the final content without adding instructions to the client conversation. Codex rejects streaming, multimodal parts, tools, other response formats, unknown additive fields, and Chat Completions generation parameters instead of silently ignoring them. The deployment-configured underlying Codex model is never taken from the client request.
