...
- The MVP accepts non-streaming, text-only `POST /v1/chat/completions` requests, maps the exact client message conversation into the isolated transport turn, and does not add OpenKeyboard operation instructions.
- `response_format: {"type":"json_object"}` validates that the final content is one JSON object without changing the client conversation. OpenKeyboard's `temperature` and `max_tokens` compatibility fields are accepted as no-ops because the CLI transport has no matching controls.
...
4. Preserve authentication ordering, rate limiting, explicit Codex grants, exact client-message mapping, request cancellation, and existing Ollama/Apfel routing. Application operation metadata may be accepted for compatibility but must not cause prompt injection or message rebuilding.
5. Reject unsupported streaming, tools, multimodal content, response formats, generation controls, and additive fields rather than overstating compatibility. The documented exceptions are JSON-object response validation and OpenKeyboard's `temperature` plus `max_tokens` no-op compatibility fields.
6. Add deterministic focused tests for every behavior change. At minimum, retain coverage for default-off behavior, configuration validation, model discovery and ownership, explicit authorization, routing, exact message mapping, response wrapping, JSON-object validation, the production OpenKeyboard request shape, safe failures, timeout, cancellation, bounded output, concurrency saturation, and Ollama/Apfel regressions. Add direct runner tests before claiming that temporary-directory removal is verified.
