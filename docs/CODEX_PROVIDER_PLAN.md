...
- Configuration bounds request duration, concurrency, queue length, prompt size, and output size. Client cancellation reaches the isolated turn and temporary files are removed afterward.
...
6. Add deterministic focused tests for every behavior change. At minimum, retain coverage for default-off behavior, configuration validation, model discovery and ownership, explicit authorization, routing, request mapping, response wrapping, structured operations, safe failures, timeout, cancellation, cleanup, bounded output, concurrency saturation, and Ollama/Apfel regressions.
7. Do not read the primary checkout's ignored configuration during deterministic tests. Any explicitly authorized credential-gated check must use the shared worktree-safe local-config route and must not retain response bodies, credentials, Authorization headers, or private prompts.
...
5. Inspect sanitized operational signals for crashes, timeouts, overload, process leaks, repeated cleanup failures, or secret exposure. Expand beyond the canary only after the named owner accepts the evidence.
6. Roll back immediately if gateway health, existing providers, authentication, latency bounds, cleanup, or the Codex probe fails. Disable the Codex configuration and canary key first when that safely contains the failure, then restore the recorded version using the tested rollback command.
